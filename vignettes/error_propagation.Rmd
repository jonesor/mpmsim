...
Sample size is critical in determining the precision of statistical estimates. In demographic studies, small sample sizes can lead to high variability in estimates of measures like $\lambda$, making it difficult to make strong inferences. Larger sample sizes reduce this variability, as seen in the narrower confidence intervals around $\lambda$ when we increase the sample size from 20 (calculated above) to 100. The following example demonstrates the effect of sample size on population growth rate estimates using simulated demographic matrices.
...
# Loop through each sample size and calculate the confidence interval for lambda
...
arrows(ci_results$sample_size, ci_results$ci_lower, ci_results$sample_size, ci_results$ci_upper,
...
This vignette demonstrates how sampling error propagates through MPMs, influencing metrics like population growth rate, and provides a practical method for estimating confidence intervals (CIs) for these metrics using `compute_ci`. By applying tools like `compute_ci`, users can also evaluate the effect of sample size on estimate precision, using this information to optimize data collection efforts and improve the reliability of demographic estimates.

## Addendum

The `compute_ci` function is intended for metrics that rely on the full **A** matrix. However, some metrics, like `life_expect_mean` from the `Rage` package, only require the **U** matrix. For these metrics, users should use the `compute_ci_U` function, which works similarly to `compute_ci`, but is specifically designed for metrics that focus on survival and growth processes within the **U** matrix.
