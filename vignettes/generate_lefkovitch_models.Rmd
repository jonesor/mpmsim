...
In all these archetypes, reproduction is placed in the top row of the matrix. In Takada et al's paper, reproduction was always placed in the top right of the MPM, meaning that only the "last" stage of the life cycle reproduced. This approach can be relaxed to allow reproduction from any stage.

## 2. Generate a random Lefkovitch MPM based on an archetype

In `mpmsim` the function `rand_lefko_mpm` can be used to generate a random MPM that conforms to one of the above four life cycle archetypes. The function allows for the generation of random MPMs based on these archetypes, with survival and growth (the U matrix) based on draws from a Dirichlet distribution to ensure biological plausibility. For flexibility, the function is quite flexible with reproductive output

The function is straightforward, in the following example, I create a three-stage MPM, based on Archetype 2. I set reproduction, arbitrarily to be 5. By default, if only a single value is given, this is placed in the top-right hand corner of the **F** matrix. Also, by default, all reproduction is assumed to be sexual.

```{r}
rand_lefko_mpm(n_stages = 3, reproduction = 5, archetype = 2)
```

To allow a random element to reproduction, users can provide reproductive output information as a list of two matrices of numeric values, both with the same dimension as `n_stages`, to provide lower and upper limits of mean reproduction for the entire matrix model. reproductive output values are then drawn from a uniform distribution between the two values. Users should use 0 for both lower and upper limits in cases with no reproduction.
