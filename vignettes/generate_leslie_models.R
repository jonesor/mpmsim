...
# Compute reproduction using the step model
stepMod <- baseDF %>%
...
plotA <- ggplot(stepMod, aes(x = x, y = reproduction)) +
  geom_line() +
  xlab("Age") +
  ylab("Reproduction") +
  ggtitle("A) Step")

# Compute reproduction using the logistic model
logisticMod <- baseDF %>%
  mutate(reproduction = model_reproduction(
...
plotB <- ggplot(logisticMod, aes(x = x, y = reproduction)) +
  geom_line() +
  xlab("Age") +
  ylab("Reproduction") +
  ggtitle("B) Logistic")

# Compute reproduction using the von Bertalanffy model
...
plotC <- ggplot(logisticMod, aes(x = x, y = reproduction)) +
  geom_line() +
  xlab("Age") +
  ylab("Reproduction") +
  ggtitle("C) von Bertalanffy")

# Compute reproduction using the normal model
normalMod <- baseDF %>%
  mutate(reproduction = model_reproduction(
    age = x, params = c(A = 10, mu = 4, sd = 2), maturity = 0,
    model = "normal"
  ))
plotD <- ggplot(normalMod, aes(x = x, y = reproduction)) +
  geom_line() +
  xlab("Age") +
  ylab("Reproduction") +
  ggtitle("D) Normal")

# Compute reproduction using the Hadwiger model
hadwigerMod <- data.frame(x = 0:50) %>%
  mutate(reproduction = model_reproduction(
    age = x, params = c(a = 0.91, b = 3.85, C = 29.78),
    maturity = 0, model = "hadwiger"
  ))
plotE <- ggplot(hadwigerMod, aes(x = x, y = reproduction)) +
  geom_line() +
  xlab("Age") +
  ylab("Reproduction") +
  ggtitle("E) hadwiger")
